...
function withRouterPropsContext(WrappedComponent) {
  return function WithRouterPropsContext(props) {
    ...
    var prevFilters = _react2.default.useRef(null);

    var fetchNewDataFunc = function fetchNewDataFunc(filter) {
      var prev = JSON.stringify(prevFilters.current);
      var current = JSON.stringify(filter);

      var isPopAction = currentRouterProps.location.action === "POP";
      var isRestoreScroll = currentRouterProps.location.state && currentRouterProps.location.state.restoreScroll === true;
